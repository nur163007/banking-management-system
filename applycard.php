...
$cardquery = "select * from card order by card_no desc limit 1";
 $cardoutput = mysqli_query($conn,$cardquery);
	$cardcount=mysqli_fetch_array($cardoutput);
	$lastid = $cardcount['card_no'];
	if($lastid == ""){
		$cardid = (rand('1000000000', '1111111111'));
	}
	else{
		$cardid = substr($lastid,10);
...
if(isset($_POST['submit'])){
	...
	$query="select account_no from customer_account where account_no='$accountno'";
	...
	while($row = mysqli_fetch_array($pquery)){
	 ...
	 if($accountno== $caccount){
		 $cardapp ="INSERT INTO card(account_no,card_no,card_type,card_limit,remark,status) VALUES('$accountno','$cardno','$cardtype','$cardlimit','$remark','$status')";
				$transect = mysqli_query($conn, $cardapp);
				if($transect){
					echo '<script>alert ("Request Sent for Card application.")</script>';
			...
			while($res = mysqli_fetch_array($result)){  
			  ?>
	                    ...
	                    ?>
						 ...
						 <div class="col-lg-8 col-12 contentoption"id="card">
						 <form action=""method="post">
						 
						 <div class="payee-form col-lg-8 col-12">
						 <h2 class="payee-head">Card Application form</h2>
						 </div>
						 <div class="row">
						 ...
						 <div class="input-payee col-lg-10 col-12 offset-lg-1 mt-3">
						 <input type="text"class="form-control"name="card_no" maxlength="16" value="<?php echo $cardid;?>">
						 </div>
                       ...
                       <option value="prepaid card">Prepaid Card</option>               
						...
						<select id="cardlimit"name="card_limit"class="form-control">
                      
                       <option value=""selected="selected">--Select card limit--</option>
                       ...
                       <option value="100000">100000</option>               
						 ...
						 <div class="input-payee col-lg-10 col-12 offset-lg-1 mt-3">
						 <input type="text"class="form-control"name="remark"value="application" readonly>
						 </div>
						 
						 
						 <div class="input-btn col-lg-4 col-12 offset-lg-5 mt-3">
						 <input type="submit"class="submit-button"name="submit"value="Apply Card">
						 </div>
