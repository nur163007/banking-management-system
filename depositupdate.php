...
$mail->Subject = 'Balance deposit confirmation';
$mail->Body    = "<h3>Banking Management Sysetm<br>Your deposit process is successfully done.<br>please check your account balance.</h3>";

$mail->send();
	}
	catch(Exception $e){
		echo '<script>alert ("Something wrong to deposit")</script>';
	}
   }
   
   
$update = "UPDATE customer_fund SET deposit = 'Accepted' WHERE account_no = '$accountno'";
...
if($result){
	    ...
	    $addsum=mysqli_query($conn,$foradd);
			  while($addition = mysqli_fetch_array($addsum)){
				...
				$transection="update transection set deposit='Accepted' where transection_no= '$tran_no'";
				$transect = mysqli_query($conn, $transection);
				
				if($transect==true){
					$uptodate ="UPDATE customer_fund set amount='$tranbalance', term='$tranterm' where transection_no='$tran'";
					$fundupdate = mysqli_query($conn, $uptodate);
				}
				
                echo '<script>alert ("Succesfully deposit balance")</script>';
