...
if(isset($_POST['submit'])){
	...
	$query="select account_no,customer_name,phone_no from customer_account where account_no='$accountno'";
	$pquery=mysqli_query($conn,$query);
	while($row = mysqli_fetch_array($pquery)){
     
	 $caccount = $row['account_no'];
	 $name = $row['customer_name'];
	 $phoneac = $row['phone_no'];
	 if($accountno== $caccount && $cname== $name && $phoneno== $phoneac){
		 $forwith ="select * from customer_fund where account_no='$accountno'";
	    $withdraw=mysqli_query($conn,$forwith);
			  while($withdrawl = mysqli_fetch_array($withdraw)){
				 $statuscheck = $withdrawl['withdrawl'];
				  $transectno =  $withdrawl['transection_no'];
				   $total2 = 0;
				   $deposit= "N/A";
				   $transfer= "N/A";
				   $term = $withdrawl['term'];
			       $with = $withdrawl['amount'];
				   $amount = $with-$money;
				 ...
				 $updatewith ="update customer_fund set withdrawl='$status',remark2='$remark',branch='$branch' where account_no='$accountno'";
			      $successwith = mysqli_query($conn, $updatewith);
		           echo '<script>alert ("Request Sent for withdrawl balance, balance withdrawl within 1/2 working days.")</script>';
			       if($successwith == true){

		        $totalbalance =$total2+$with-$money;
				$transection="INSERT INTO transection(remark,debit,credit,balance,phone_no,term,deposit,transfer,withdrawl,transection_no) VALUES('$remark','$money','$credit','$totalbalance','$phoneno','$term','$deposit','$transfer','$status','$transectno')";
...
if ($conn->connect_error) {
			...
			while($res = mysqli_fetch_array($result)){  
			  ?>
	                    ...
	                    ?>
						 ...
						 <div class="col-lg-8 col-12 contentoption"id="withdraw">
  ...
  <div class="calc-row">
    <div class="button zero">0</div><div class="button decimal">.</div><div class="button">+</div><div class="button equal">=</div>
						 ...
						 <h2 class="payee-head">Fund Withdrawl Part</h2>
						 </div>
						 <div class="row">
						 
						 <div class="input-payee col-lg-10 col-12 offset-lg-1 mt-3">
						 <input type="text"class="form-control"name="customer_name"placeholder="Enter account holder name">
						 </div>
                       ...
                       <option value="Uttara Branch">Uttara Branch</option>
						 ...
						 <div class="input-payee col-lg-10 col-12 offset-lg-1 mt-3">
						 <input type="text"class="form-control"name="remark"value="withdrawl" readonly>
						 </div>
						 
						 
						 <div class="input-btn col-lg-4 col-12 offset-lg-4 mt-3">
						 <input type="submit"class="submit-button"name="submit"value="Withdraw Amount">
						 </div>
