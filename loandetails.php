			...
			while($output = mysqli_fetch_array($result)){  
			  ?>
	                    ...
	                    ?>
						 ...
						 <div class="col-lg-8 col-12 contentoption"id="loandet">
						     <div class="container-fluid mt-3">
						 
						 <div class="account-request-payee col-12">
						 <h2 class="text-left text-danger">All Approved loan</h2>
						 </div>
						 <!--account details part start -->
						 <input type="text" name="" class="form-control"id="myInput" onkeyup="searchFunction()"placeholder="Search by Loan Type">
...
if ($conn->connect_error) {
    die("Connection failed: " . $conn->connect_error);
	  ...
	  $sql ="select * from loan where phone_no = '$phoneno' and status='Approved'";
	  $query = mysqli_query($conn,$sql);
      while($res = mysqli_fetch_array($query)) {
             $slno=$res['serial_no'];
			$loansearch="select * from active_loan where serial_no='$slno' and status='Approved'";
			$search=mysqli_query($conn, $loansearch);
			while($lfind=mysqli_fetch_array($search)){
				
?>
			    <tbody>
				
			    <tr class="text-center bg-white">
				<td> <?php echo $lfind['sl_no'];?></td>
				<td> <?php echo $lfind['loan_type'];?></td>
				<td> <?php echo '$'.$lfind['amount'];?></td>
				<td> <?php echo $lfind['start_date'];?></td>
				<td> <?php echo $lfind['end_date'];?></td>
				<td> <?php echo $lfind['duration'];?></td>
				<td> <?php echo $lfind['payable_amount'];?></td>
				<td><?php echo '$'.$lfind['emi_m'].' '.'(monthly)'.'<br>'.' '.'$'.$lfind['emi_q'].' '.'(quarterly)'.'<br>'.' '.'$'.$lfind['emi_y'].' '.'(yearly)';?></td>
				<td><a href="seeloan.php?serial_no=<?php echo $lfind['serial_no'];?>&&sl_no=<?php echo $lfind['sl_no'];?>"class="text-success"style="text-decoration:none;font-size:20px;">View</a></td>
				</tr>
